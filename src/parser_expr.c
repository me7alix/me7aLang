...
AST_Node *expr_expand(AST_Nodes *nodes) {
	...
	for (size_t i = 0; i < nodes->count; i++) {
		...
		bool isBinOp = node->kind == AST_BIN_EXP && !(node->expr_binary.l && node->expr_binary.r);
		bool isUnOp = node->kind == AST_UN_EXP && !node->expr_unary.v;
...
Type expr_analysis(Parser *p, AST_Node *expr, Type *vart) {
	...
	case AST_BIN_EXP: {
		Type lt = expr_analysis(p, expr->expr_binary.l, vart);
		...
		if (expr->expr_binary.op == AST_OP_FIELD) {
			if (expr->expr_binary.r->kind == AST_METHOD_CALL) {
				...
				da_foreach (StructMember, member, &mems) {
					if (member->kind == STMEM_METHOD) {
						if (strcmp(expr->expr_binary.r->method_call.id,
							...
							metCall->method_call.args.items[0] = expr->expr_binary.l;
							metCall->method_call.type = expr->expr_binary.type;
				...
				if (lt.kind == TYPE_POINTER) {
					expr->expr_binary.l = ast_new(
						.kind = AST_UN_EXP,
						.expr_unary.op = AST_OP_DEREF,
						.expr_unary.v = expr->expr_binary.l,
						.expr_unary.type = *lt.pointer.base,
					);
	...
	case AST_UN_EXP: {
		...
		case AST_OP_SIZEOF: {
			expr_analysis(p, expr->expr_unary.v, NULL);
		} break;

		case AST_OP_CAST: {
			expr_analysis(p, expr->expr_unary.v, &expr->expr_unary.type);
		} break;
		...
		default:
			expr->expr_unary.type = expr_analysis(p, expr->expr_unary.v, vart);
		}
	...
	while (true) {
		...
		case TOK_ID:
			if (peek2(p).kind == TOK_OPAR) {
				if (nodes.count > 0) {
					if (da_last(&nodes)->kind == AST_BIN_EXP &&
							da_last(&nodes)->expr_binary.op == AST_OP_FIELD) {
		...
		case TOK_SIZEOF:
			if (peek2(p).kind == TOK_COL) {
				da_append(&nodes, ast_new(
					.kind = AST_UN_EXP,
					.loc = peek(p).loc,
					.expr_unary.op = tok_to_unary_expr_op(next(p)),
				...
				da_append(&nodes, ast_new(
					.kind = AST_UN_EXP,
					.loc = peek(p).loc,
					.expr_unary.op = tok_to_unary_expr_op(peek(p)),
					.expr_unary.type = TUPTR,
					.expr_unary.v = NULL,
				));
			}
		...
		case TOK_OSQBRA:
			da_append(&nodes, ast_new(
				.kind = AST_BIN_EXP,
				.loc = peek(p).loc,
				.expr_binary.op = tok_to_binary_expr_op(peek(p).kind),
				.expr_binary.l = NULL,
				.expr_binary.r = NULL
			));
			next(p);
		...
		case TOK_STAR: case TOK_MINUS: {
			...
			if (nodes.count == 0) {
				...
				if (da_last(&nodes)->kind == AST_UN_EXP) {
					...
					if (isBinOp && da_last(&nodes)->expr_binary.l &&
						da_last(&nodes)->expr_binary.r) isBinOp = false;
