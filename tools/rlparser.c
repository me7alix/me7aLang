...
typedef enum { DEFAULT = 0, JSON, XML, LUA, CODE, M7 } OutputFormat;
...
int main(int argc, char *argv[])
{
    ...
    else if (outputFormat == M7) printf("\nOutput format:    M7\n\n");
...
static void ShowCommandLineInfo(void)
{
    ...
    printf("                                      Supported types: DEFAULT, JSON, XML, LUA, CODE, M7\n\n");
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (IsTextEqual(argv[i], "-f", 2) || IsTextEqual(argv[i], "--format", 8))
        {
            if (((i + 1) < argc) && (argv[i + 1][0] != '-'))
            {
                ...
                else if (IsTextEqual(argv[i + 1], "M7\0", 3)) outputFormat = M7;
            }
